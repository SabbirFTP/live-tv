...
const PUBLIC_SOURCES = [
  { label:'Monjil (Pro)',      url:'https://raw.githubusercontent.com/Monjil404/livetv/refs/heads/main/pro',                                                                tag:null },
  { label:'Iptv-Org (BD)',     url:'https://raw.githubusercontent.com/iptv-org/iptv/master/streams/bd.m3u',                                                                tag:null },
  { label:'Iptv-Org (IN)',     url:'https://raw.githubusercontent.com/iptv-org/iptv/master/streams/in.m3u',                                                                tag:null },
  { label:'Free-TV (Global)',  url:'https://raw.githubusercontent.com/Free-TV/IPTV/master/playlist.m3u8',                                                                  tag:null },
  { label:'Bugsfree',          url:'https://raw.githubusercontent.com/Bugsfree-Hosts/Bugs-free-IPTV/main/channel.m3u',                                                     tag:null },
  { label:'time2shine (BD)',   url:'https://raw.githubusercontent.com/time2shine/bbc/refs/heads/main/bbc',                                                                  tag:null },
  { label:'Axsport',           url:'https://raw.githubusercontent.com/byte-capsule/Toffee-Channels-Link-Headers/main/toffee_OTT_Navigator.m3u', tag:'sports', hint:'referrer-locked' },
...
// ─── LocalStorage ─────────────────────────────────────────────────────────────
function saveStreams()  { try { localStorage.setItem(LS_STREAMS, JSON.stringify(streams)); } catch(e){} }
function loadStreams()  { try { const r = localStorage.getItem(LS_STREAMS); return r ? JSON.parse(r).filter(s=>s&&s.url) : []; } catch(e){ return []; } }
function saveTheme(d)   { try { localStorage.setItem(LS_THEME, d?'dark':'light'); } catch(e){} }
function getVisitCount(){ try { const n=parseInt(localStorage.getItem(LS_VISITS)||'2644',10)+1; localStorage.setItem(LS_VISITS,String(n)); return n; } catch(e){ return 2644; } }

// ─── URL helpers ─────────────────────────────────────────────────────────────
const SOCIAL_RE = /^https?:\/\/(?:[^/]*\.)?(?:t\.me|telegram\.(?:me|org|dog)|wa\.me|whatsapp\.com|fb\.com|facebook\.com|youtu\.be)/i;
const STATIC_RE = /\.(?:jpe?g|png|gif|webp|svg|ico|css|js|txt|md|html?|php)(?:[?#]|$)/i;
const MEDIA_DL  = /\.(?:mkv|avi|wmv|flv|mov|iso|divx|mp3|aac|zip|rar|exe|apk|pdf)(?:[?#]|$)/i;

function isFiltered(url){ return SOCIAL_RE.test(url)||STATIC_RE.test(url)||MEDIA_DL.test(url); }

function githubRaw(url){
  return url.replace(/^https?:\/\/github\.com\/([^/]+)\/([^/]+)\/(?:raw|blob)\/(.+)$/i,
...
function extractName(url){
  try {
    ...
    return (p[p.length-1]||p[p.length-2]||u.hostname)
      .replace(/\.(m3u8?|ts|mpd)$/i,'').replace(/[-_]/g,' ').trim() || u.hostname;
...
function parseInput(text, limit=1500){
  ...
  for(const raw of lines){
    ...
    if(line.startsWith('#EXTINF')){
      ...
      meta = { name:(nm?nm[1].trim():''), logo:(lg?lg[1]:''), group:(gr?gr[1]:''), tvgName:(tn?tn[1]:'') };
    } else if(/^(https?|rtmps?|rtsp):\/\//i.test(line)){
      if(!isFiltered(line) && !seen.has(line)){
        seen.add(line);
        result.push({ url:line, name:meta?.name||meta?.tvgName||extractName(line), logo:meta?.logo||'', group:meta?.group||'', status:'idle' });
        meta = null;
        if(result.length >= limit) break;
      } else { meta = null; }
    } else if(!line.startsWith('#')){
      const found = line.match(/https?:\/\/[^\s'"<>()]+?\.(?:m3u8?|mpd|ts)(?:\?[^\s'"<>()]*)?/gi);
      if(found){ found.forEach(u=>{ if(!isFiltered(u)&&!seen.has(u)&&result.length<limit){ seen.add(u); result.push({url:u,name:meta?.name||extractName(u),logo:meta?.logo||'',group:meta?.group||'',status:'idle'}); } }); meta=null; }
    }
  }
  return result;
}
...
async function checkStream(url, signal){
  ...
  if(location.protocol==='https:'&&/^http:\/\//i.test(url))
    return {status:'blocked', type:'Mixed-Content', proto:'HTTP', responseTime:'0ms'};

  const isHls=/\.m3u8?(\?|$)/i.test(url);
  const type = isHls?'HLS':/\.mpd(\?|$)/i.test(url)?'DASH':/\.ts(\?|$)/i.test(url)?'Direct TS':'Stream';
  ...
  try {
    ...
    await fetch(url,{method:'HEAD',signal:ctrl.signal,mode:'no-cors',cache:'no-store'}).catch(e=>{if(e.name==='AbortError')throw e; return null;});
    clearTimeout(tid);

    if(isHls){
      try {
        const gc=new AbortController(); const gt=setTimeout(()=>gc.abort(),5000);
        const gr=await fetch(url,{signal:gc.signal,cache:'no-store'});
        clearTimeout(gt);
        const txt=await gr.text();
        if(!/#EXTM3U|#EXT-X/i.test(txt.slice(0,500)))
          return {status:'dead',type,proto,responseTime:(Date.now()-t0)+'ms'};
      } catch {}
    }
    return {status:'playable',type,proto,responseTime:(Date.now()-t0)+'ms'};
  } catch(e){
    if(e.name==='AbortError') throw e;
    const rt=Date.now()-t0;
    return {status: rt<7500?'blocked':'dead', type, proto, responseTime:rt+'ms'};
  }
}
...
async function checkBulk(list, onProgress, signal){
  ...
  async function worker(i){
    ...
    try { const r=await checkStream(list[i].url,signal); Object.assign(list[i],r); }
    catch { list[i].status='dead'; }
    done++;
    onProgress(done, list.length, i);
  }
  for(let i=0;i<list.length;i+=CONCURRENCY){
    if(signal?.aborted) break;
    await Promise.all(Array.from({length:Math.min(CONCURRENCY,list.length-i)},(_,k)=>worker(i+k)));
  }
}
...
function playStream(stream){
  ...
  if(stream.name){ nameEl.textContent=stream.name; nameEl.classList.remove('hidden'); }
  else { nameEl.classList.add('hidden'); }

  if(stream.logo){
    logoEl.style.cssText='';
    const img=document.createElement('img');
    img.src=stream.logo; img.alt=stream.name||''; img.className='channel-logo';
    img.onerror=()=>img.remove();
    logoEl.innerHTML=''; logoEl.appendChild(img); logoEl.classList.remove('hidden');
  } else if(stream.name){
    logoEl.textContent=stream.name.charAt(0).toUpperCase();
    logoEl.classList.remove('hidden'); logoEl.style.cssText='';
  } else { logoEl.classList.add('hidden'); }

  liveBadge.classList.add('hidden');
  infoType.textContent='…'; infoStatus.textContent='Loading…'; infoProto.textContent='…'; infoResp.textContent='…';

  const url=stream.url;
  const isHls=/\.m3u8?(\?|$)/i.test(url)||!/\.(mp4|ts|webm)(\?|$)/i.test(url);
  const proto=/^https/i.test(url)?'HTTPS':'HTTP';
  const type=/\.m3u8/i.test(url)?'HLS':/\.mpd/i.test(url)?'DASH':'Stream';
  infoType.textContent=type; infoProto.textContent=proto;
  ...
  if(isHls && window.Hls && window.Hls.isSupported()){
    ...
    hls.on(window.Hls.Events.ERROR,(_,d)=>{
      if(d.fatal){ infoStatus.textContent='Error'; overlay.classList.remove('hidden'); }
    });
  } else if(video.canPlayType('application/vnd.apple.mpegurl')){
    video.src=url; video.play().catch(()=>{});
    video.addEventListener('loadedmetadata',()=>{
      overlay.classList.add('hidden'); liveBadge.classList.remove('hidden');
      infoStatus.textContent='Live ●'; infoResp.textContent=(Date.now()-t0)+'ms';
    },{once:true});
  } else {
    video.src=url; video.play().catch(()=>{});
    overlay.classList.add('hidden'); infoStatus.textContent='Playing';
  }
...
const CARD_MAP={playable:'playable-card',blocked:'blocked-card',dead:'dead-card'};
...
function buildCard(stream, idx, animated=true){
  ...
  const groupHtml=stream.group?`<span class="group-badge">${escHtml(stream.group)}</span>`:'';
  const respHtml=(stream.responseTime&&stream.responseTime!=='—')
...
function renderList(){
  ...
  const visible=streams
    ...
    .filter(({s})=>{
      ...
      if(searchQuery){ const q=searchQuery.toLowerCase(); return (s.name||'').toLowerCase().includes(q)||(s.url||'').toLowerCase().includes(q)||(s.group||'').toLowerCase().includes(q); }
      return true;
    });
...
function updateSelectBar(){
  ...
  if(allBox){
    allBox.classList.toggle('checked', selectedIdxs.size===streams.length&&streams.length>0);
  }
}

// ─── Source Pills Renderer ────────────────────────────────────────────────────
function renderSources(){
  const wrap=document.getElementById('sourcesWrap'); if(!wrap) return;
  wrap.innerHTML=PUBLIC_SOURCES.map(src=>{
    const tagHtml=src.tag?`<span class="src-tag ${src.tag}">${src.tag}</span>`:'';
    const hintHtml=src.hint?`<span class="hint-badge">${escHtml(src.hint)}</span>`:'';
...
function startBulkCheck(list){
  ...
  checkBulk(list, (done, total, idx)=>{
    ...
    if(el){ el.outerHTML=buildCard(streams[idx],idx,false); }
    updateStats();
  }, checkAbort.signal).then(()=>{
    if(!checkAbort.signal.aborted){
      progWrap.classList.add('hidden'); bulkProg.classList.add('hidden');
      saveStreams(); renderList(); showToast('✅ চেক সম্পন্ন!');
    }
  });
}
...
function initDisclaimer(){
  ...
  if(!modal) return;
  try { if(localStorage.getItem(LS_DISCLAIMER)==='1'){ modal.style.display='none'; return; } } catch(e){}
  modal.style.display='flex';
  document.getElementById('disclaimerOk')?.addEventListener('click',()=>{
    try{ localStorage.setItem(LS_DISCLAIMER,'1'); }catch(e){}
...
function initPlayerControls(){
  ...
  document.getElementById('pcPlay')?.addEventListener('click',()=>{
    ...
    if(video.paused){ video.play(); btn.innerHTML='<svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="5" width="4" height="14" rx="1"/><rect x="14" y="5" width="4" height="14" rx="1"/></svg>'; }
    else { video.pause(); btn.innerHTML='<svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>'; }
  });
  ...
  // Volume
  document.getElementById('pcVol')?.addEventListener('input',e=>{ video.volume=parseFloat(e.target.value); });

  // PiP
  document.getElementById('pcPip')?.addEventListener('click',async()=>{
    try { document.pictureInPictureElement ? await document.exitPictureInPicture() : await video.requestPictureInPicture(); } catch(e){}
  });

  // Fullscreen (both buttons)
  function doFullscreen(){
    const el=document.getElementById('playerSection')||video;
    try { !document.fullscreenElement ? (el.requestFullscreen?.())||el.webkitRequestFullscreen?.() : document.exitFullscreen?.(); } catch(e){}
  }
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  document.getElementById('btnCheckSingle')?.addEventListener('click',async()=>{
    const url=(document.getElementById('singleUrl')?.value||'').trim(); if(!url) return;
    const btn=document.getElementById('btnCheckSingle');
    btn.disabled=true; btn.innerHTML='<span class="pill-spin"></span>';
    try {
      const r=await checkStream(url);
      showToast(r.status==='playable'?'✅ Stream চলবে!':r.status==='blocked'?'🔒 Blocked (CORS/Mixed)':'❌ Dead / সাড়া নেই');
    } catch { showToast('চেক করতে সমস্যা'); }
    finally { btn.disabled=false; btn.innerHTML='<svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>Check'; }
  });
  ...
  document.getElementById('btnBulkCheck')?.addEventListener('click',()=>{
    ...
    if(!parsed.length){ showToast('কোনো বৈধ URL পাওয়া যায়নি'); return; }
    streams=parsed.map(s=>({...s,status:'checking'}));
    selectedIdxs.clear(); saveStreams(); renderList();
  ...
  // ── Source Pills ──────────────────────────────────────────────────────────
  document.getElementById('sourcesWrap')?.addEventListener('click',async e=>{
    // copy URL button inside pill
    const copyBtn=e.target.closest('[data-copy-src]');
    if(copyBtn){ e.stopPropagation(); await copy(copyBtn.dataset.copySrc); showToast('URL কপি হয়েছে!'); return; }

    const pill=e.target.closest('.source-pill'); if(!pill) return;
    if(document.getElementById('sourcesWrap').classList.contains('busy')) return;

    const url=pill.dataset.srcUrl; const label=pill.dataset.srcLabel;
    ...
    try {
      ...
      const limit=(label.includes('Bugsfree')||label.includes('time2shine'))?1500:undefined;
  ...
  document.getElementById('btnCopyAllSources')?.addEventListener('click',async()=>{
  ...
  searchClear?.addEventListener('click',()=>{ searchBox.value=''; searchQuery=''; searchClear.classList.add('hidden'); renderList(); });

  // ── Stream List event delegation ──────────────────────────────────────────
  document.getElementById('streamList')?.addEventListener('click',async e=>{
    const card=e.target.closest('.stream-card'); if(!card) return;
    const idx=parseInt(card.dataset.idx,10);

    // Play
    if(e.target.closest('[data-play]')){ playStream(streams[idx]); return; }
    ...
    if(e.target.closest('[data-check-single]')){
      ...
      const tagEl=card.querySelector('.tag'); if(tagEl){ tagEl.className='tag tag-checking'; tagEl.textContent='… Checking'; }
      try { const r=await checkStream(streams[idx].url); Object.assign(streams[idx],r); saveStreams(); }
      catch { streams[idx].status='dead'; }
      card.outerHTML=buildCard(streams[idx],idx,false); updateStats(); return;
    }

    // Copy URL
    if(e.target.closest('[data-copy-url]')){ await copy(streams[idx].url); showToast('URL কপি হয়েছে!'); return; }

    // Delete
    if(e.target.closest('[data-del]')){
      streams.splice(idx,1); selectedIdxs.delete(idx);
      // renumber selectedIdxs above idx
      const newSel=new Set(); selectedIdxs.forEach(i=>{ if(i<idx) newSel.add(i); else if(i>idx) newSel.add(i-1); }); selectedIdxs=newSel;
      saveStreams(); renderList(); return;
    }
    ...
    if(e.target.closest('[data-check]')||e.target===card){
      ...
      if(chkEl||e.target===card){
        ...
        const chk=card.querySelector('.checkbox-custom'); if(chk) chk.classList.toggle('checked',selectedIdxs.has(idx));
  ...
  document.getElementById('selectAllBox')?.addEventListener('click',()=>{
    if(selectedIdxs.size===streams.length){ selectedIdxs.clear(); }
    else { streams.forEach((_,i)=>selectedIdxs.add(i)); }
    renderList();
  });

  document.getElementById('selectBarClose')?.addEventListener('click',()=>{ selectedIdxs.clear(); renderList(); });
  ...
  document.getElementById('btnCopySelectedM3U')?.addEventListener('click',async()=>{
    const sel=streams.filter((_,i)=>selectedIdxs.has(i));
    if(!sel.length) return;
    await copy(buildM3U(sel)); showToast('✅ M3U কপি হয়েছে!');
  });
  ...
  // ── Export buttons ────────────────────────────────────────────────────────
  document.getElementById('btnCopyPlayable')?.addEventListener('click',async()=>{
    const live=streams.filter(s=>s.status==='playable');
    if(!live.length){ showToast('কোনো live stream নেই'); return; }
    await copy(buildM3U(live)); showToast(`✅ ${live.length}টি live M3U কপি হয়েছে!`);
  });

  document.getElementById('btnExport')?.addEventListener('click',()=>{
    if(!streams.length) return; downloadM3U(streams,'live-tv-pro.m3u');
  });

  document.getElementById('btnDeleteBlocked')?.addEventListener('click',()=>{
    streams=streams.filter(s=>s.status!=='blocked'); selectedIdxs.clear(); saveStreams(); renderList(); showToast('Blocked stream মুছে ফেলা হয়েছে');
  });

  document.getElementById('btnDeleteDead')?.addEventListener('click',()=>{
    streams=streams.filter(s=>s.status!=='dead'); selectedIdxs.clear(); saveStreams(); renderList(); showToast('Dead stream মুছে ফেলা হয়েছে');
  });
